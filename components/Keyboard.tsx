...
const Keyboard: React.FC<KeyboardProps> = ({ targetChar, activeCode, language, theme, errorHeatmap = new Set() }) => {
  ...
  return (
    <div className="w-[90vw] mx-auto pb-2 px-2 select-none">
      <div className={`flex flex-col gap-[0.2vw] p-[0.6vw] rounded-xl border shadow-2xl backdrop-blur-md transition-colors ${isDark ? 'bg-slate-900/60 border-slate-800' : 'bg-slate-200/60 border-slate-300'}`}>
              ...
              return (
                <div
                  ...
                  className={`
                    ...
                    ${keyObj.width || 'w-full'} h-full border
                    ${isActive 
                      ? 'bg-blue-500 text-white scale-[0.96] shadow-inner border-transparent' 
                      : isTarget 
                        ? 'bg-amber-500 text-white shadow-[0_0_1vw_rgba(245,158,11,0.5)] ring-2 ring-amber-400 border-transparent' 
                        : isError
